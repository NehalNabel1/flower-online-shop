...
export const order_add_get = async (request, response) => {
    try {
...
export const order_add_post = async (request, response) => {
    try {
        ...
        const user = await AuthModel.getUserByUserId(sessionOrder.userId);
        if (!user) {
            return response.redirect('/order');
        }
        ...
        if (sessionOrder.items.length === 1 && sessionOrder.cartId) {
            ...
            const deletedItem = await CartModel.deleteItemFromCart(sessionOrder.cartId);
            if (!deletedItem) {
            ...
            const deletedCart = await CartModel.ClearCart(sessionOrder.userId);
...
export const order_delete_post = async (request, response) => {
    try {
