...
cartRouter.post('/',authGuards.isAuth ,
    check('amount').not().isEmpty().withMessage('Amount is required').bail()
    .isInt({min :1}).withMessage('Amount must be greater than 0'),
     cartController.cart_add_post)

cartRouter.post('/save', authGuards.isAuth , 
    check('amount').not().isEmpty().withMessage('Amount is required').bail()
    .isInt({min : 1}).withMessage('Amount must be greater than 0'),
cartController.cart_save_post);
