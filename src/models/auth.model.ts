...
export const login =async(email : string , password : string) : Promise <{id : Types.ObjectId , isAdmin : boolean} | null>=>{
    try{
        //check for email 
       const user = await User.findOne({email});
       if(!user){
       ...
       //check for password
       const same = await bcrypt.compare(password , user.password as string)
       if(!same){
